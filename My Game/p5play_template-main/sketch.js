...
function setup() {
  ...
  walls = new Group()
  boundary1 = createSprite(width/2,100,width-180,20);
  walls.add(boundary1)
  boundary2 = createSprite(100,height/2,20,height-180);
  walls.add(boundary2)
  boundary3 = createSprite(width-100,height/2,20,height-180);
  walls.add(boundary3)
  boundary4 = createSprite(width/4-120,height-100,boundary1.width/3,20);
  walls.add(boundary4)
  boundary5 = createSprite((width-width/4)+120,height-100,boundary1.width/3,20);
  walls.add(boundary5)
  wall1 = createSprite(width/4,350,350,15);
  wall1.addImage(wallImg);
  wall1.scale=0.45;
  walls.add(wall1);
  wall2 = createSprite(width-width/4,350,350,15);
 wall2.addImage(wallImg);
  wall2.scale=0.4;
  walls.add(wall2)
  wall3 = createSprite(width/4,height-363,350,15);
  wall3.addImage(wallImg);
  wall3.scale=0.4;
  walls.add(wall3);
  wall4 = createSprite(width-width/4,height-363,350,15);
...
function handleMotoin(){
 ...
 function drawScore(){
  ...
  textSize(25)
  text("Timer :  "+ timer+" seconds",180,200)
  
 }
function checkTimer(){
  console.log();
  if(Math.round(millis()/1000)>0){
   timer=timer-1;
  }
}
